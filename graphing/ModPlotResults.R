...
if (!exists("kLabelSmidgeFactor")) {
  # Start with Dark2 from Brewer palette and add color-blind palette so there are more entries -- note that names are not used in the code
  kLabelSmidgeFactor <- 0.6
}

if (!exists("kLabelVerticalDistance")) {
  # Start with Dark2 from Brewer palette and add color-blind palette so there
  # are more entries -- note that names are not used in the code
  kLabelVerticalDistance <- 0.1
}
...
if (!exists("kLineThickness")) {
    kLineThickness <- 0.5
}

if (!exists("kPlotAxisFontSize")) {
    kPlotAxisFontSize <- 22
}
...
PlotGuessingCurves <- function(lookup.results,
  ...
  plot.plus.axes <- (plot.plus.axes +
                     theme(axis.title.y = element_text(size=24, hjust=0.2)))
  plot.plus.axes <- (plot.plus.axes +
                     theme(axis.text.x=element_text(size=kPlotAxisFontSize)))
  plot.plus.axes <- (plot.plus.axes +
                     theme(axis.text.y=element_text(size=kPlotAxisFontSize)))
